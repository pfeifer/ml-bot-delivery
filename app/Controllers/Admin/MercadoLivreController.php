...
class MercadoLivreController extends BaseController
{
    ...
    public function credentialForm($id = null)
    {
        ...
        if ($id) {
            ...
            if ($cred) {
                // Desencripta o client_secret ANTES de enviar para a view
                try {
                    $cred->client_secret = $cred->client_secret ? Services::encrypter()->decrypt($cred->client_secret) : '';
                } catch (Throwable $e) {
                    log_message('error', "Erro ao desencriptar client_secret para edição (ID: {$id}): " . $e->getMessage());
                    // Deixa em branco por segurança
                    $cred->client_secret = ''; 
                }
    ...
    public function saveCredentials()
    {
        ...
        $encryptedSecret = $encrypter->encrypt($this->request->getPost('client_secret'));
